...
const AddMenuItem = () => {
  ...
  useEffect(() => {
    ...
    if (!loading && !user) {
      navigate('/login', { state: { returnTo: '/add-menu-item' } });
    }
    // Redirect regular users to home
    else if (!loading && user && userType !== 'user1') {
      navigate('/');
      toast({
        title: 'Access Denied',
        description: 'Only mess workers can add menu items.',
        variant: 'destructive',
      });
    }
  }, [user, loading, navigate, userType, toast]);
      ...
      <main className="flex-1 container mx-auto px-4 md:px-6 pt-24 pb-12">
        <div className="mb-8 text-center md:text-left">
          ...
          <p className="text-muted-foreground md:text-lg max-w-2xl mx-auto md:mx-0">
            Create a new menu item to display in the menu.
          </p>
        </div>
